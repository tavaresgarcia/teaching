# Tutorial 6: Modelling Volatility - II
...
mydata <- read.delim("Merck(1).csv", header = TRUE,  sep = ",")
        ...
        for (thresh in 0:1){
          ...
          if (ph == 0 && qh == 0){
            if (!thresh){ # no such thing as a homoscedastic threshold ARMA
              # for models with constant variance, the ugarchspec and ugarchfit
              # functions do not work well; instead, the documentation advises
              # to use arfimaspec and arfimafit
              try(silent = T, expr = {
...
# max value in the lag ACFs
max(sacf_tgarch[,6:15])
min(sacf_tgarch[,6:15])

# Autocorrelations appear to be relatively small for all models.

# (b)
#
# plotting estimated volatilities is the same as for basic GARCH (we still don't
...
# TGARCH vs GARCH does not make a noticeable difference for fixed pm, qm, ph and
# qh. However, larger pm and qm appear to be associated with larger "spikes" in
# volatilities when they do occur.

# (c)
#
# To test for leverage effects, we focus on the threshold coefficient in models
# where the threshold is actually included. Note that rugarch calls the
# threshold parameter gamma, whereas we call it lambda.
for (i in 1:nmods){
...
# the null of "no leverage effects" is easily rejected in favour of "leverage
# effects" for all models except ARMA(2,1)-TGARCH(1,1), where "reverse effects"
# are being confirmed at a very low significance level.
...
# (a)
#
# same approach as with GARCH/TGARCH models; here it is somewhat simpler because
# we fix all GARCH lags to be 1.
...
# max value in the lag ACFs
max(sacf_garchm[,4:13])
min(sacf_garchm[,4:13])
...
# All volatility estimates generally look very similar.
# GARCHM vs GARCH appears to exhibit only a minor difference for fixed p, q. The
# periods of high volatility match up to those estimated using TGARCH models.
...
# (d)
# To forecast volatilities, we again use the ugarchboot function.
