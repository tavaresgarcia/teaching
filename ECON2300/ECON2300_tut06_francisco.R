...
LM <- read_csv("lead_mortality.csv") %>%
  mutate(lead_ph = lead*ph, 
         lead_ph_65 = lead(ph - 6.5))
attach(LM)

reg1 <- lm_robust(infrate ~ lead, se_type = "stata")
reg2 <- lm_robust(infrate ~ lead + ph + lead_ph, se_type = "stata")
reg3 <- lm_robust(infrate ~ lead + ph + lead_ph_65, se_type = "stata")
...
fig8.1 <- ggplot(LM, aes(x = ph, y = infrate, col = as.factor(lead)))+
  ...
  geom_smooth(data = LM, method = "lm", se = FALSE, linewidth = 1) +
  theme(axis.title = element_text(family = "serif"),
        ...
        legend.background = element_rect(fill = "lightgrey", linewidth = 0.8,
                                         linetype = "solid")) +
