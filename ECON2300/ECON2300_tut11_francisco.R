...
Names <- read_csv("Names.csv") %>%
  mutate(female.black = female*black, high.black = high*black)
attach(Names)


## (a)
reg1 = lm_robust(call_back ~ black, se_type = "stata")
summary(reg1)
...
## (b)
reg2 = lm_robust(call_back ~ black * female, se_type = "stata")
summary(reg2)
# From (2) in the table, the call-back rate for male blacks 0.0887 - 0.0304 =
# 0.0583, and for female blacks is 0.0887 - 0.0304 + 0.0102 - 0.0022 = 0.0663.
# The interaction term is -0.002239, which is not significant at the 5% level
# (t-statistic = -0.12).

## (c)
reg3 = lm_robust(call_back ~ high, se_type = "stata")
summary(reg3)

reg4 = lm_robust(call_back ~ black * high, se_type = "stata")
summary(reg4)
...
# load data
tsls <- read_csv("tsls.csv")
attach(tsls)


## (a)
reg1 = lm_robust(y ~ x1 + x2, se_type = "stata")
summary(reg1)
