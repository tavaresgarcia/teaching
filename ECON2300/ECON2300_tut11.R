...
reg2 = lm_robust(call_back ~ black + female.black, se_type = "stata")
reg3 = lm_robust(call_back ~ high, se_type = "stata")
reg4 = lm_robust(call_back ~ black + high + high.black, se_type = "stata")
...
# Table 1 in page 2 provides answers to (a)-(c).
## (a)
summary(reg1)
...
ols.1stage = lm_robust(x2 ~ x1 + z1)
tsls$x2_hat <- predict(ols.1stage, newdata = data.frame(x1 = x1, z1 = z1))
summary(ols.1stage)
cor(x2, z1)
